    ...
    <div class="d-flex align-content-start flex flex-wrap">
      <div v-if="!isRootPath" v-on:click="levelUp" class="fm-grid-item text-center">
        <div class="fm-item-icon">
          <i class="fa fa-level-up fa-5x pb-2" />
        </div>
      ...
      >
        <div class="fm-item-icon">
          <i
            class="fa-5x pb-2"
            v-bind:class="(acl && directory.acl === 0) ? 'fa fa-unlock-alt' : 'fa fa-folder'"
          />
        </div>
      ...
      >
        <div class="fm-item-icon">
          <i v-if="acl && file.acl === 0" class="fa fa-unlock-alt fa-5x pb-2" />
          <thumbnail v-else-if="thisImage(file.extension)" v-bind:disk="disk" v-bind:file="file"></thumbnail>
          <i v-else class="fa fa-5x pb-2" v-bind:class="extensionToIcon(file.extension)" />
        </div>
        <div class="fm-item-info">
