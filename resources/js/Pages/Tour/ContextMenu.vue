...
export default {
  ...
  mounted() {
     ...
     * 'contextMenu'
     */
    Bus.$on("contextMenuTour", (item, event) => {
      console.log(event);
      this.context = item;
      this.showMenu(event);
    });
  ...
  methods: {
    ...
    showMenu(event) {
      if (this.context) {
        ...
        // focus on menu
        this.$nextTick(() => {
          this.$refs.contextMenuTour.focus();
          // set menu params
    ...
    setMenu(top, left) {
      ...
      const el = this.$refs.contextMenuTour.parentNode;
      ...
      const maxY =
        elY +
        (el.offsetHeight - this.$refs.contextMenuTour.offsetHeight - 25);
      const maxX =
        elX +
        (el.offsetWidth - this.$refs.contextMenuTour.offsetWidth - 25);
